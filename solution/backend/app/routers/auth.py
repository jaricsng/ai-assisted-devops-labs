...
async def login(payload: LoginRequest, db: AsyncSession = Depends(get_db)):
    ...
    if not user or not verify_password(payload.password, user.hashed_password):
        logger.info("audit", action="LOGIN_FAILED", resource="user", email=payload.email)
...
async def logout(request: Request, user: User = Depends(current_user)):
    ...
    if jti:
        revoke_token(jti)
    logger.info("audit", action="LOGOUT", resource="session", resource_id=jti, user_id=user.id)
