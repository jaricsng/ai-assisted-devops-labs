...
_EXAMPLE_KEYS = {"change-me", "secret", "your-secret-key", "test-secret-key-for-local-dev-only"}
    ...
    def validate_secret_key_in_production(self) -> "Settings":
        if self.environment == "production":
            ...
            if self.secret_key.lower() in _EXAMPLE_KEYS:
                raise ValueError("SECRET_KEY must not be an example or placeholder value in production.")
        return self
