...
def user_payload():
    ...
    return {"email": f"alice_{unique}@example.com", "full_name": "Alice", "password": "Alice123!"}
...
class TestProtectedEndpoints:
    ...
    async def test_token_missing_sub_claim_returns_401(self, client: AsyncClient):
        ...
        token = jose_jwt.encode(no_sub_payload, settings.secret_key, algorithm=settings.algorithm)
        resp = await client.get("/projects", headers={"Authorization": f"Bearer {token}"})
        assert resp.status_code == 401
