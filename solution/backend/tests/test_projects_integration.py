...
async def test_get_project_by_id(client: AsyncClient):
    ...
    project_id = (await client.post("/projects", json={"name": "Fetchable"}, headers=headers)).json()["id"]
...
async def test_get_nonexistent_project_returns_404(client: AsyncClient):
    token = await _register_and_login(client)
    resp = await client.get("/projects/999999", headers={"Authorization": f"Bearer {token}"})
...
async def test_delete_project_returns_204(client: AsyncClient):
    ...
    project_id = (await client.post("/projects", json={"name": "Deletable"}, headers=headers)).json()["id"]
...
async def test_deleted_project_not_returned(client: AsyncClient):
    ...
    project_id = (await client.post("/projects", json={"name": "Gone"}, headers=headers)).json()["id"]
...
async def test_delete_nonexistent_project_returns_404(client: AsyncClient):
    token = await _register_and_login(client)
    resp = await client.delete("/projects/999999", headers={"Authorization": f"Bearer {token}"})
    assert resp.status_code == 404
...
async def test_idor_user_b_cannot_read_user_a_project(client: AsyncClient):
    ...
    project_id = (
        await client.post("/projects", json={"name": "Secret"}, headers={"Authorization": f"Bearer {token_a}"})
    ).json()["id"]

    resp = await client.get(f"/projects/{project_id}", headers={"Authorization": f"Bearer {token_b}"})
    assert resp.status_code == 404
...
async def test_idor_user_b_cannot_delete_user_a_project(client: AsyncClient):
    ...
    project_id = (
        await client.post("/projects", json={"name": "Protected"}, headers={"Authorization": f"Bearer {token_a}"})
    ).json()["id"]

    resp = await client.delete(f"/projects/{project_id}", headers={"Authorization": f"Bearer {token_b}"})
    assert resp.status_code == 404
