...
async def test_login_returns_access_token(client):
    email = _email()
    await client.post("/auth/register", json={"email": email, "full_name": "T", "password": _PASS})
    response = await client.post("/auth/login", json={"email": email, "password": _PASS})
...
async def test_login_wrong_password_returns_401(client):
    email = _email()
    await client.post("/auth/register", json={"email": email, "full_name": "T", "password": _PASS})
    response = await client.post("/auth/login", json={"email": email, "password": "wrongPass1"})
