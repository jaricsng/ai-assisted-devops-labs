...
async def test_list_tasks(client: AsyncClient):
    headers, pid = await _setup(client)
    await client.post(f"/projects/{pid}/tasks", json={"title": "T1", "priority": "LOW"}, headers=headers)
    await client.post(f"/projects/{pid}/tasks", json={"title": "T2", "priority": "LOW"}, headers=headers)
...
async def test_get_task(client: AsyncClient):
    headers, pid = await _setup(client)
    task_id = (
        await client.post(f"/projects/{pid}/tasks", json={"title": "Fetch Me", "priority": "LOW"}, headers=headers)
    ).json()["id"]
...
async def test_update_task_title(client: AsyncClient):
    headers, pid = await _setup(client)
    task_id = (
        await client.post(f"/projects/{pid}/tasks", json={"title": "Old", "priority": "LOW"}, headers=headers)
    ).json()["id"]

    resp = await client.patch(f"/projects/{pid}/tasks/{task_id}", json={"title": "New"}, headers=headers)
    assert resp.status_code == 200
...
async def test_status_transition_todo_to_in_progress(client: AsyncClient):
    headers, pid = await _setup(client)
    task_id = (
        await client.post(f"/projects/{pid}/tasks", json={"title": "Move Me", "priority": "MEDIUM"}, headers=headers)
    ).json()["id"]

    resp = await client.patch(f"/projects/{pid}/tasks/{task_id}", json={"status": "IN_PROGRESS"}, headers=headers)
    assert resp.status_code == 200
...
async def test_status_transition_full_happy_path(client: AsyncClient):
    headers, pid = await _setup(client)
    task_id = (
        await client.post(f"/projects/{pid}/tasks", json={"title": "Full Path", "priority": "MEDIUM"}, headers=headers)
    ).json()["id"]

    for next_status in ("IN_PROGRESS", "IN_REVIEW", "DONE"):
        resp = await client.patch(f"/projects/{pid}/tasks/{task_id}", json={"status": next_status}, headers=headers)
...
async def test_invalid_status_transition_returns_422(client: AsyncClient):
    headers, pid = await _setup(client)
    task_id = (
        await client.post(f"/projects/{pid}/tasks", json={"title": "Bad Jump", "priority": "LOW"}, headers=headers)
    ).json()["id"]

    resp = await client.patch(f"/projects/{pid}/tasks/{task_id}", json={"status": "DONE"}, headers=headers)
...
async def test_terminal_state_cannot_transition(client: AsyncClient):
    headers, pid = await _setup(client)
    task_id = (
        await client.post(f"/projects/{pid}/tasks", json={"title": "Terminal", "priority": "LOW"}, headers=headers)
    ).json()["id"]

    await client.patch(f"/projects/{pid}/tasks/{task_id}", json={"status": "CANCELLED"}, headers=headers)
    resp = await client.patch(f"/projects/{pid}/tasks/{task_id}", json={"status": "TODO"}, headers=headers)
    assert resp.status_code == 422
...
async def test_delete_task_soft_deletes(client: AsyncClient):
    headers, pid = await _setup(client)
    task_id = (
        await client.post(f"/projects/{pid}/tasks", json={"title": "Doomed", "priority": "LOW"}, headers=headers)
    ).json()["id"]

    assert (await client.delete(f"/projects/{pid}/tasks/{task_id}", headers=headers)).status_code == 204
    assert (await client.get(f"/projects/{pid}/tasks/{task_id}", headers=headers)).status_code == 404


async def test_create_and_list_comment(client: AsyncClient):
    headers, pid = await _setup(client)
    task_id = (
        await client.post(f"/projects/{pid}/tasks", json={"title": "Discuss", "priority": "LOW"}, headers=headers)
    ).json()["id"]
    ...
    list_resp = await client.get(f"/projects/{pid}/tasks/{task_id}/comments", headers=headers)
...
async def test_tasks_in_wrong_project_return_404(client: AsyncClient):
    ...
    task_id = (
        await client.post(f"/projects/{pid}/tasks", json={"title": "Wrong Project", "priority": "LOW"}, headers=headers)
    ).json()["id"]
...
async def _create_task(client: AsyncClient, headers: dict, pid: int, title: str = "Private Task") -> int:
    return (
        await client.post(
            f"/projects/{pid}/tasks",
...
async def test_filter_tasks_by_status(client: AsyncClient):
    ...
    headers, pid = await _setup(client)
    await client.post(f"/projects/{pid}/tasks", json={"title": "Todo Task", "priority": "LOW"}, headers=headers)
    task_id = (
        await client.post(f"/projects/{pid}/tasks", json={"title": "Progress Task", "priority": "LOW"}, headers=headers)
    ).json()["id"]
    await client.patch(f"/projects/{pid}/tasks/{task_id}", json={"status": "IN_PROGRESS"}, headers=headers)
...
async def test_filter_tasks_by_priority(client: AsyncClient):
    ...
    headers, pid = await _setup(client)
    await client.post(f"/projects/{pid}/tasks", json={"title": "High Task", "priority": "HIGH"}, headers=headers)
    await client.post(f"/projects/{pid}/tasks", json={"title": "Low Task", "priority": "LOW"}, headers=headers)
...
async def test_filter_tasks_by_status_and_priority(client: AsyncClient):
    ...
    headers, pid = await _setup(client)
    await client.post(f"/projects/{pid}/tasks", json={"title": "High Todo", "priority": "HIGH"}, headers=headers)
    await client.post(f"/projects/{pid}/tasks", json={"title": "Low Todo", "priority": "LOW"}, headers=headers)
    await client.post(f"/projects/{pid}/tasks", json={"title": "High Progress", "priority": "HIGH"}, headers=headers)

    resp = await client.get(f"/projects/{pid}/tasks?status=TODO&priority=HIGH", headers=headers)
    assert resp.status_code == 200
    tasks = resp.json()
    assert len(tasks) == 2
...
async def test_filter_tasks_no_match_returns_empty_list(client: AsyncClient):
    ...
    headers, pid = await _setup(client)
    await client.post(f"/projects/{pid}/tasks", json={"title": "Only Task", "priority": "LOW"}, headers=headers)
...
async def test_create_comment_emits_audit_log(client: AsyncClient):
    ...
    assert resp.status_code == 201
    audit = [e for e in cap if e.get("event") == "audit" and e.get("action") == "COMMENT_CREATED"]
    assert len(audit) == 1
