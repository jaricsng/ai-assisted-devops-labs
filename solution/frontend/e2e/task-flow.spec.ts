...
test.describe("Task flow — critical user journey", () => {
  ...
  test("user can move task through full lifecycle to DONE", async ({ page }) => {
    const token = await loginAsNewUser(page);
    const detail = new ProjectDetailPage(page);

    // Create project and task via API for speed
    const projectRes = await page.request.post("http://localhost:8000/projects", {
      headers: { Authorization: `Bearer ${token}` },
      data: { name: "Full Lifecycle Project" },
    });
    const project = await projectRes.json();

    await page.request.post(`http://localhost:8000/projects/${project.id}/tasks`, {
      headers: { Authorization: `Bearer ${token}` },
    ...
    await expect(page.getByTestId("kanban-board")).toBeVisible();
    await expect(detail.column("TODO").getByText("Lifecycle Task")).toBeVisible();

    await detail.moveTask("→ In Progress");
    await expect(detail.column("IN_PROGRESS").getByText("Lifecycle Task")).toBeVisible();

    await detail.moveTask("→ In Review");
    await expect(detail.column("IN_REVIEW").getByText("Lifecycle Task")).toBeVisible();

    await detail.moveTask("→ Done");
    await expect(detail.column("DONE").getByText("Lifecycle Task")).toBeVisible();
  });

  test("user can cancel a task from the TODO state", async ({ page }) => {
    const token = await loginAsNewUser(page);
    const detail = new ProjectDetailPage(page);

    const projectRes = await page.request.post("http://localhost:8000/projects", {
      headers: { Authorization: `Bearer ${token}` },
      data: { name: "Cancel Test Project" },
    });
    const project = await projectRes.json();

    await page.request.post(`http://localhost:8000/projects/${project.id}/tasks`, {
      headers: { Authorization: `Bearer ${token}` },
      data: { title: "Task To Cancel" },
    });

    await page.goto(`/projects/${project.id}`);
    await expect(page.getByTestId("kanban-board")).toBeVisible();
    await expect(detail.column("TODO").getByText("Task To Cancel")).toBeVisible();

    await detail.moveTask("→ Cancelled");
    await expect(detail.column("CANCELLED").getByText("Task To Cancel")).toBeVisible();
    await expect(detail.column("TODO").getByText("Task To Cancel")).not.toBeVisible();
  });
