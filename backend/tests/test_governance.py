...
async def test_project_name_too_long_returns_422(client):
    ...
    response = await client.post("/projects", json={"name": "x" * 256}, headers=headers)
...
@pytest.mark.asyncio
async def test_task_description_too_long_returns_422(client):
    """Task description > 2000 chars must be rejected by Pydantic StringConstraints."""
    headers = await _register_and_login(client)
    pid = (
        await client.post("/projects", json={"name": "Gov Project 3"}, headers=headers)
    ).json()["id"]
    response = await client.post(
        f"/projects/{pid}/tasks",
        json={"title": "Valid Title", "description": "x" * 2001, "priority": "LOW"},
        headers=headers,
    )
    assert response.status_code == 422
...
@pytest.mark.asyncio
async def test_user_full_name_too_long_returns_422(client):
    """User full_name > 255 chars must be rejected by Pydantic StringConstraints."""
    response = await client.post(
        "/auth/register",
        json={
            "email": _email(),
...
async def test_audit_log_on_project_create(client):
    ...
    assert any(
        e.get("action") == "PROJECT_CREATED" for e in audit_events
...
async def test_audit_log_on_project_delete(client):
    ...
    audit_events = [e for e in cap if e.get("event") == "audit"]
    assert any(
        e.get("action") == "PROJECT_DELETED" for e in audit_events
...
async def test_audit_log_on_task_create(client):
    ...
    pid = (
        await client.post(
            "/projects", json={"name": "Audit Task Project"}, headers=headers
        )
    ).json()["id"]
    with capture_logs() as cap:
        resp = await client.post(
    ...
    audit_events = [e for e in cap if e.get("event") == "audit"]
    assert any(
        e.get("action") == "TASK_CREATED" for e in audit_events
...
async def test_audit_log_on_logout(client):
    ...
    audit_events = [e for e in cap if e.get("event") == "audit"]
    assert any(
        e.get("action") == "LOGOUT" for e in audit_events
...
async def test_audit_log_on_gdpr_delete(client):
    ...
    audit_events = [e for e in cap if e.get("event") == "audit"]
    assert any(
        e.get("action") == "USER_DELETED" for e in audit_events
...
async def test_audit_log_on_register(client):
    ...
    audit_events = [e for e in cap if e.get("event") == "audit"]
    assert any(
        e.get("action") == "REGISTER" for e in audit_events
...
async def test_audit_log_on_login_success(client):
    ...
    audit_events = [e for e in cap if e.get("event") == "audit"]
    assert any(
        e.get("action") == "LOGIN_SUCCESS" for e in audit_events
...
async def test_audit_log_on_login_failed(client):
    ...
    audit_events = [e for e in cap if e.get("event") == "audit"]
    assert any(
        e.get("action") == "LOGIN_FAILED" for e in audit_events
...
async def test_audit_log_on_task_update(client):
    ...
    pid = (
        await client.post(
            "/projects", json={"name": "Audit Update Project"}, headers=headers
        )
    ).json()["id"]
    tid = (
        await client.post(
    ...
    audit_events = [e for e in cap if e.get("event") == "audit"]
    assert any(
        e.get("action") == "TASK_UPDATED" for e in audit_events
...
async def test_audit_log_on_task_delete(client):
    ...
    pid = (
        await client.post(
            "/projects", json={"name": "Audit Delete Project"}, headers=headers
        )
    ).json()["id"]
    tid = (
        await client.post(
    ...
    audit_events = [e for e in cap if e.get("event") == "audit"]
    assert any(
        e.get("action") == "TASK_DELETED" for e in audit_events
