...
async def test_user_id_bound_to_audit_log_context(client):
    ...
    await client.post(
        "/auth/register",
        json={"email": email, "full_name": "Obs", "password": "Obs12345!"},
    )
    login = await client.post(
        "/auth/login", json={"email": email, "password": "Obs12345!"}
    )
    ...
    assert (
        len(audit_events) >= 1
    ), f"no audit events; captured events: {[e.get('event') for e in cap]}"
    assert (
        "user_id" in audit_events[0]
...
def test_configure_logging_production_uses_json_renderer():
    ...
    *original* list. Restoring via configure_logging("test") would leave cached
    ...
    try:
        configure_logging(
            "production"
        )  # creates a new list; cached loggers keep original_list
        processor_types = [
            type(p).__name__ for p in structlog.get_config()["processors"]
        ]
        assert (
            "JSONRenderer" in processor_types
...
async def test_metrics_middleware_counts_slow_requests(client):
    ...
    from app.middleware.metrics import SLOW_REQUEST_THRESHOLD_MS, _counters
    ...
    assert (
        _counters.slow_requests > initial_slow
